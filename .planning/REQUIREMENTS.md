...
- [x] **MAPS-03**: Place metadata (rating, types, opening_now where available) is shown inline on the decision row in the dashboard when present; absent if the Places lookup didn't return it. No styling regression to existing rows that lack metadata.
