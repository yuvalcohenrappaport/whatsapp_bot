...
export async function parseTravelIntent(
  ...
  const systemInstruction = `You are a travel assistant parsing @mention requests from a WhatsApp group. Determine if the message is travel-related. If yes, extract all available details and build an optimized Google search query. If essential info is missing (especially destination), set isVague=true and provide a clarification question in ${langLabel}. Consider the recent group context to infer missing details (e.g., if the group discussed a trip to Barcelona, a "find hotels" request should auto-fill Barcelona as destination). Respond in JSON.`;
